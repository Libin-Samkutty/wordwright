 ...
 */
const repository = createSettingsRepository(reportStorageIssue);

export function SettingsProvider({ children }: { children: ReactNode }): React.JSX.Element {
  const [settings, setSettings] = useState<Settings>(() => repository.read());
  ...
  useEffect(() => repository.subscribe(setSettings), []);
